...
class FileSearchPage(QWidget):
    ...
    def search_files(self):
        ...
        if not self.directory:
            self.results_box.setPlainText("Please select a folder first.")
            return

        query = self.search_input.text().strip()

        if not query:
            self.results_box.setPlainText("Please enter something to search for.")
            return
        ...
        self.results_box.setPlainText("Searching files...")

        self.worker = FileSearchWorker(query, self.directory)
    ...
    def show_error(self, error):
        self.results_box.setPlainText(f"Error: {error}")
        self.finish_search()
